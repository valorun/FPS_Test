...
public class WeaponController : MonoBehaviour
{
	...
	public event System.Action OnAmmoChangeEvent;
    ...
    private void NetworkStart()
    {
        ...
        if (np.networkObject.IsOwner)
        {
			...
			foreach (var weapon in weaponList) {
				if (weapon.worldModel != null) {
					SetLayerRecursively (weapon.worldModel, 8);
				}
				//hide the player model as well
				SetLayerRecursively(np.PlayerModel, 8);
			}
        ...
        else
        {
			foreach (var weapon in weaponList) {
				if (weapon.viewModel != null) {
					SetLayerRecursively (weapon.viewModel, 8);
				}
			}
        }
    }
    ...
    void Reload()
    {
        ...
        if (np.networkObject.IsOwner)
        {
			...
			if (currentWeapon.reloadSound != null) {
				np.PlayerAudioSource.PlayOneShot (currentWeapon.reloadSound);
			}
        }
    ...
    void Update()
    {
        ...
        if (currentWeapon != weaponList[0])
        {
			...
			//If the player shot at least one bullet he can reload
			if (Input.GetButton("Reload") && !isReloading && currentWeapon.bulletsPerClip < weaponList[currentWeaponIndex].bulletsPerClip)
			{
				Reload();
			}    
        }
            ...
            if (currentWeapon.bulletsPerClip > 0 && !isReloading)
            {
                ...
                if (Time.time > nextShotTime)
                {
					...
					if (currentWeapon.shootSound != null) {
						np.PlayerAudioSource.PlayOneShot (currentWeapon.shootSound);
					}
