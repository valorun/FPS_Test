...
public class HUDSystem : MonoBehaviour
{
    ...
    //the respawn counter
    [SerializeField]
	private Text respawnCounterText;
    ...
    void Start()
    {
		...
		wp.OnAmmoChangeEvent += UIOnAmmoChange;

		//Find the network player as well and hook up events
    ...
    /// <summary>
	/// Called from the event on WeaponController
    /// </summary>
    private void UIOnAmmoChange()
    {
        //only do HUD on the owner
        if (np.networkObject.IsOwner)
        {  
            //set the ammo text
			string aText = wp.CurrentWeapon.bulletsPerClip + " - " + wp.CurrentWeapon.clips;
			ammoText.text = aText;
        }
